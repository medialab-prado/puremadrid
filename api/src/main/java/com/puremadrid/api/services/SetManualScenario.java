...
public class SetManualScenario extends MainServlet {

    private static final Logger mLogger = Logger.getLogger(SetManualScenario.class.getName());
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (resultList.size() == amount) {
            ...
            entity.setIndexedProperty(MainServlet.PROPERTY_ESCENARIO_STATE_TOMORROW_MANUAL, escenario.name());
            entity.setIndexedProperty(MainServlet.PROPERTY_ESCENARIO_STATE_TOMORROW, escenario.name());

            datastore.put(entity);

            boolean warningEstado = false;
            NotificationUtils.sendNotification(warningEstado, (String) entity.getProperty(PROPERTY_AVISO), (String)  entity.getProperty(PROPERTY_AVISO_STATE), (String)  entity.getProperty(PREPERTY_AVISO_MAX_TODAY), (String) entity.getProperty(PROPERTY_ESCENARIO_STATE_TODAY), (String) entity.getProperty(PROPERTY_ESCENARIO_STATE_TOMORROW), Constants.FLAG_SET_ALWAYS_TOMORROW);

            mLogger.info("Updated Manual Scenario for Tomorrow");
